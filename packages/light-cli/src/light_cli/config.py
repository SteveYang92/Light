...
import yaml
from light_asr import AsrEngine
...
__all__ = ["AsrEngine", "SubtitleConfig"]
...
class SubtitleConfig:
    ...
    # ── Per-stage config adapters (field mapping for light_subtitle) ──

    def segment_config(self) -> SegmentConfig:
        return SegmentConfig(max_duration=self.max_duration)
    ...
    def translate_config(self) -> TranslateConfig:
        return TranslateConfig(
            ...
            max_duration=self.max_duration,
            evaluate_enabled=self.evaluate_enabled,
    ...
    def layout_config(self) -> LayoutConfig:
        return LayoutConfig(
            cps_limit=self.cps_limit,
            cps_limit_en=self.cps_limit_en,
            max_lines=self.max_lines,
            max_lines_zh=self.max_lines_zh,
            max_chars_per_line_zh=self.max_chars_per_line_zh,
            max_chars_per_line_en=self.max_chars_per_line_en,
            min_duration=self.min_duration,
            max_duration=self.max_duration,
            reading_padding=self.reading_padding,
            optimize_entry_points=self.optimize_entry_points,
            target_lang=self.target_lang,
            llm_temperature=self.llm_temperature,
        )
