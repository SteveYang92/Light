...
from light_llm.usage.report import load_usage_from_dir, merge_reports, write_usage_report
from light_llm.usage.tracker import USAGE_REPORT_FILENAME
...
def _copy_single_segment(output_dir: Path, seg_dir: Path, slug: str) -> None:
    ...
    for suffix in sidecar_suffixes:
        src = sub_artifacts.find_sidecar(seg_dir, suffix) or (seg_dir / suffix)
        if not src.exists():
            continue
        dst = sub_artifacts.sidecar_path(output_dir, suffix)
        if dst.exists():
            dst.unlink()
