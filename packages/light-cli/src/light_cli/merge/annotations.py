...
def _merge_annotations_ass(
    ...
    has_any = any(sub_artifacts.find_sidecar(seg, "annotations.ass") for seg in seg_dirs)
    if not has_any:
        return
    ...
    for k, seg in enumerate(seg_dirs):
        ass_path = sub_artifacts.find_sidecar(seg, "annotations.ass")
        if ass_path is None:
            continue
        offset = offsets[k]
        seg_dur = durations[k]
    ...
    out = sub_artifacts.sidecar_path(output_dir, "annotations.ass")
...
def _merge_annotations_vtt(
    ...
    has_any = any(sub_artifacts.find_sidecar(seg, "annotations.vtt") for seg in seg_dirs)
    if not has_any:
        return

    for k, seg in enumerate(seg_dirs):
        src = sub_artifacts.find_sidecar(seg, "annotations.vtt") or (seg / "annotations.vtt")
        cues = _parse_vtt(src)
        offset = offsets[k]
        seg_dur = durations[k]
    ...
    out = sub_artifacts.sidecar_path(output_dir, "annotations.vtt")
