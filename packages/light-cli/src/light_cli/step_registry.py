...
stage under :mod:`light_cli.steps`; resume-time state hydration lives
in :mod:`light_cli.state_hydrate`.
"""
...
from .artifacts import asr_words_path, audio_wav_path
...
def _subtitle_artifact_paths(config: SubtitleConfig) -> tuple[Path, ...]:
    if config.target_lang:
        return (
            sub_artifacts.raw_cues_path(_out(config)),
            sub_artifacts.segment_json_path(_out(config)),
            sub_artifacts.plan_json_path(_out(config)),
        )
    return (
        sub_artifacts.segment_json_path(_out(config)),
        sub_artifacts.plan_json_path(_out(config)),
...
def build_step_definitions(config: SubtitleConfig) -> list[StepDefinition]:
    ...
    return [
        ...
        StepDefinition(
            ...
            artifacts=lambda c: (sub_artifacts.segment_json_path(_out(c)),),
        ...
        StepDefinition(
            ...
            artifacts=lambda c: (sub_artifacts.segment_json_path(_out(c)), sub_artifacts.plan_json_path(_out(c))),
        ...
        StepDefinition(
            ...
            artifacts=lambda c: (sub_artifacts.plan_json_path(_out(c)),),
        ...
        StepDefinition(
            ...
            artifacts=lambda c: (sub_artifacts.partial_cues_path(_out(c)),),
            hydrate=translate._hydrate_translate_mid,
        ...
        StepDefinition(
            ...
            artifacts=lambda c: (sub_artifacts.raw_cues_path(_out(c)),),
