...
from light_cli.video_split import SilenceInterval, _best_silence, should_split


class TestShouldSplit:
    def test_short_video_returns_false(self) -> None:
        with patch("light_cli.video_split.probe_duration", return_value=120.0):
            assert should_split(Path("short.mp4"), threshold=2700) is False

    def test_long_video_returns_true(self) -> None:
        with patch("light_cli.video_split.probe_duration", return_value=3600.0):
            assert should_split(Path("long.mp4"), threshold=2700) is True

    def test_exactly_threshold(self) -> None:
        with patch("light_cli.video_split.probe_duration", return_value=2700.0):
            assert should_split(Path("exact.mp4"), threshold=2700) is False

    def test_custom_threshold(self) -> None:
        with patch("light_cli.video_split.probe_duration", return_value=600.0):
