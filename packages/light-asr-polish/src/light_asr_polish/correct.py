...
Debug artifacts (written under ``work_dir/transcript_correct/`` when *work_dir*
...
from light_llm.json_extract import extract_json_object, parse_json_array_response
...
def correct(
    ...
    correct_dir = Path(work_dir) / "transcript_correct" if work_dir is not None else None
...
def _extract_domain_context(
    ...
    """
    cache_path = correct_dir / "domain_context.json" if correct_dir is not None else None
    usage_path = correct_dir / "usage.json" if correct_dir is not None else None
