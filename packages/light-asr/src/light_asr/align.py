...
def align_words(
    ...
    Uses whisper.cpp's original segment boundaries as alignment anchors,
    read from *raw_json_path*; when ``None``, falls back to locating
    ``asr/asr_whisper-cpp.raw.json`` (or legacy ``whisper_output.json``)
    relative to *audio_path*.
    """
    import whisperx
    ...
    wcpp_json = Path(raw_json_path) if raw_json_path else _find_whisper_cpp_raw(audio_path)

    if wcpp_json is not None and wcpp_json.exists():
...
def _find_whisper_cpp_raw(audio_path: str) -> Path | None:
    """Locate whisper.cpp's raw segment JSON relative to *audio_path*."""
    output_dir = Path(audio_path).parent
    candidates = [
        output_dir / "asr" / "asr_whisper-cpp.raw.json",
        output_dir / "asr" / "whisper_output.json",
        output_dir.parent / "asr" / "asr_whisper-cpp.raw.json",
        output_dir.parent / "asr" / "whisper_output.json",
    ]
