...
class TestSplitPartParsing:
    def test_parse_split_part(self):
        assert _parse_split_part("p0059_0") == ("p0059", 0)
        assert _parse_split_part("p0059_1") == ("p0059", 1)
        assert _parse_split_part("p0187_12") == ("p0187", 12)
        assert _parse_split_part("p0059") is None
        assert _parse_split_part("mu0059_u0060_0") is None  # legacy ids no longer split

    def test_split_group_part_counts(self):
        segments = [
            _seg("p0059_0", "part zero"),
            _seg("p0059_1", "part one"),
            _seg("p0001", "whole unit"),
        ]
        assert _split_group_part_counts(segments) == {"p0059": 2}

    def test_is_last_split_part(self):
        counts = {"p0059": 2}
        assert _is_last_split_part("p0059_0", counts) is False
        assert _is_last_split_part("p0059_1", counts) is True
        assert _is_last_split_part("p0001", counts) is None
...
class TestSplitPayload:
    def test_build_payload_includes_split_group(self):
        segments = [
            _seg("p0059_0", "values for freedom being"),
            _seg("p0059_1", "corrupted by money"),
        ]
        ...
        assert translate_units[0]["split_group"] == "p0059"
...
class TestParseResponse:
    def test_binds_by_batch_index(self):
        segments = [
            _seg("p0194", "It's haunted"),
            _seg("p0195", "Peruvian government"),
        ]
        response = json.dumps(
            [
        ...
        assert len(cues) == 2
        assert cues[0].unit_id == "p0194"
        assert cues[0].text.startswith("这里闹鬼")
        assert cues[1].unit_id == "p0195"
        assert "秘鲁" in parsed[1]

    def test_fallback_unit_id_when_no_batch_index(self):
        segments = [_seg("p0001", "hello")]
        response = json.dumps([{"unit_id": "p0001", "text": "你好"}])
    ...
    def test_parse_returns_texts(self):
        segments = [
            _seg("p0187_0", "AI based"),
            _seg("p0187_1", "algorithm push"),
        ]
        response = json.dumps(
            [
...
class TestChunkPendingSegments:
    def test_keeps_split_group_in_one_batch(self):
        pending = [_seg(f"p0001_{i}", f"word{i} text") for i in range(98)] + [
            _seg("p0059_0", "first part of split"),
            _seg("p0059_1", "second part of split"),
        ]
        chunks = _chunk_pending_segments(pending, 100)
        assert len(chunks) == 1
        assert chunks[0][-2].unit_id == "p0059_0"
        assert chunks[0][-1].unit_id == "p0059_1"
    ...
    def test_never_splits_group_across_batches(self):
        pending = [_seg(f"u{i:04d}", f"text {i}") for i in range(99)] + [
            _seg("p0059_0", "first part"),
            _seg("p0059_1", "second part"),
        ]
        chunks = _chunk_pending_segments(pending, 100)
        assert len(chunks) == 2
        assert len(chunks[0]) == 99
        assert [s.unit_id for s in chunks[1]] == ["p0059_0", "p0059_1"]

    def test_oversized_group_single_batch(self):
        pending = [_seg(f"p0001_{i}", f"part {i}") for i in range(105)]
        chunks = _chunk_pending_segments(pending, 100)
        assert len(chunks) == 1
...
class TestRosolieRegression:
    """Regression guards for Paul Rosolie seg1 p0193–p0199 mis-mapping case."""

    def _segments(self) -> list[Segment]:
        return [
            _seg(item["unit_id"], item["text"], start=759.0 + i, end=760.0 + i)
            for i, item in enumerate(ROSOLIE_P0193_0199)
        ]
    ...
    def test_split_siblings_stay_in_one_chunk(self):
        ...
        chunks = _chunk_pending_segments(pending, 100)
        assert len(chunks) == 1
        split_ids = [s.unit_id for s in chunks[0] if s.unit_id.startswith("p0195")]
        assert split_ids == ["p0195_0", "p0195_1"]

    def test_batch_index_maps_haunted_to_p0194_not_peruvian(self):
        """LLM may return wrong unit_id; batch_index must bind semantics correctly."""
        segments = self._segments()
        response = json.dumps(
            [
                {"batch_index": 0, "unit_id": "p0199", "text": "有人说那里有大脚怪，别去"},
                {"batch_index": 1, "unit_id": "p0193", "text": "闹鬼什么的，像个荒诞故事"},
        ...
        assert "闹鬼" in by_id["p0194"]
        assert "秘鲁" in by_id["p0195_0"]
        assert "大脚怪" in by_id["p0193"]
