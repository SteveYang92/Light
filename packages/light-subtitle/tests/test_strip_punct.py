...
Current intended behavior (since 680cf22 "feat: preserve 顿号"):
- ，；：and mid-line 。→ single ASCII space (" "), NOT U+3000.
  ``strip_punct._FULLWIDTH_SPACE`` has been " " since the initial commit
  and all shipped subtitle outputs use ASCII spaces.
- 、is preserved (顿号 carries list semantics a space cannot convey).
"""

from __future__ import annotations

from light_models import SubtitleCue
from light_subtitle.subtitle.strip_punct import strip_chinese_punct
...
def test_period_line_end_multi_line():
    ...
    result = strip_chinese_punct(cues)
    # Trailing space trimmed per-line, line-end 。removed.
...
# ═══════════════════════════════════════════════════════════════════
# Mid-line period → ASCII space
# ═══════════════════════════════════════════════════════════════════


def test_period_mid_line_to_space():
    """。mid-line → ASCII space."""
...
def test_period_mid_line_multi_line():
    ...
    result = strip_chinese_punct(cues)
    # Mid-line 。→ " ", line-end 。removed, trailing space trimmed.
    assert result[0].text == "第一句 继续第一行\n第二行内容"


# ═══════════════════════════════════════════════════════════════════
# Comma / ；/ ：→ ASCII space; 、preserved
# ═══════════════════════════════════════════════════════════════════


def test_comma_to_space():
    cues = [_make_cue(text="第一部分，第二部分")]
    result = strip_chinese_punct(cues)
    assert result[0].text == "第一部分 第二部分"


def test_dunhao_preserved():
    """、is kept (changed from → space in 680cf22 "feat: preserve 顿号")."""
    cues = [_make_cue(text="苹果、香蕉、橘子")]
    result = strip_chinese_punct(cues)
    assert result[0].text == "苹果、香蕉、橘子"


def test_semicolon_to_space():
    cues = [_make_cue(text="已经完成；仍需改进")]
    result = strip_chinese_punct(cues)
    assert result[0].text == "已经完成 仍需改进"


def test_colon_to_space():
...
def test_mixed_mid_line_punctuation():
    """Various mid-line punctuation → ASCII space."""
...
def test_leading_space_trimmed():
    """Replacement space at line start is trimmed."""
    # "。开头" → " 开头" → strip → "开头"
...
def test_trailing_space_trimmed():
    """Replacement space at line end is trimmed."""
...
def test_all_punctuation_only():
    """Line with only replaceable punctuation → empty."""
    # 、excluded from the input: it is preserved now, not replaced.
    cues = [_make_cue(text="，。；：")]
    result = strip_chinese_punct(cues)
