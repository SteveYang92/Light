...
from light_subtitle.cli import _finished_payload, _is_tty, _make_reporter, _resolve_media_duration
...
class TestFinishedPayloadDuration:
    def test_resolves_legacy_slug_video_when_stale_path_missing(self, tmp_path: Path):
        ...
        with patch("light_subtitle.utils.ffmpeg.probe_duration", return_value=123.4) as probe:
            duration = _resolve_media_duration(work, stale, slug)
            assert duration == 123.4
            probe.assert_called_once_with(str(video))

        with patch("light_subtitle.utils.ffmpeg.probe_duration", return_value=123.4):
