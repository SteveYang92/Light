...
from light_text.punctuation import CJK_ALL_PUNCT, CJK_CLAUSE_PUNCT, CJK_PARTICLES, CJK_SENTENCE_PARTICLES
...
def _split_cue(cue: SubtitleCue, config: LayoutConfig) -> list[SubtitleCue]:
    ...
    if not text:
        return [cue]
    if cue.lang == "zh":
        return split_chinese(cue, text, max_chars_per_line=config.max_chars_per_line_zh)
    else:
        return split_english(cue, text, max_chars_per_line=config.max_chars_per_line_en, max_lines=config.max_lines)
...
def _can_stand_alone(cue: SubtitleCue, config: LayoutConfig) -> bool:
