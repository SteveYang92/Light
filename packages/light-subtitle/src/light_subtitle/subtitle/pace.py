...
def correct(cues, config: LayoutConfig, *, llm: OpenAIClient | None = None, transcript_words=None):
    ...
    *transcript_words* feeds entry-point optimization.  Returns
    ...
    ``llm=None`` disables the LLM compression pass (over-limit cues are
    kept as-is for QC to flag).
    """
    return _apply_time_corrections(cues, config, llm=llm, transcript_words=transcript_words)
...
def _apply_time_corrections(cues, config: LayoutConfig, *, llm: OpenAIClient | None = None, transcript_words=None):
    ...
    if config.target_lang:
        ...
        if over:
            compress_usage = compress.compress_over_cps(result, over, config, llm=llm)
            if compress_usage:
...
def _fix_cue_duration(cue, config: LayoutConfig):
...
def _over_cps_indices(cues, config: LayoutConfig) -> list[int]:
...
def _enforce_cps_ceiling(cues, config: LayoutConfig):
