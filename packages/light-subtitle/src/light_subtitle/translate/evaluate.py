...
Usage::
    ...
    scores, usage = evaluate_translations(translated_cues, source_segments, config, llm=client)
...
def evaluate_translations(
    ...
    *,
    ...
    Returns empty list if evaluation is disabled or no LLM client is given.
    """
    if not config.evaluate_enabled or llm is None:
        return [], None
    ...
    for batch_idx in range(0, len(pairs), EVAL_BATCH_SIZE):
        ...
        batch_scores, usage = _evaluate_batch(batch, config, batch_idx // EVAL_BATCH_SIZE, system_prompt, llm)
