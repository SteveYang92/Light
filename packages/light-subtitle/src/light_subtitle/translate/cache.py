...
def plan_units(
    ...
    *,
    ...
    translation_segments, plan_usage = plan_pipeline.run(segments, config, plan_dir, llm=llm, progress=progress)
    if plan_usage:
        save_step_usage(plan_dir / artifacts.USAGE_JSON, plan_usage)
    return translation_segments, plan_usage


def load_plan_segments(
    plan_dir: Path, segments: list[Segment], config: PlanConfig, *, llm: OpenAIClient | None = None
) -> list[Segment]:
    """Rebuild translation units from ``plan/plan.json`` when resuming mid-translate.

    Falls back to re-running the planner (LLM calls when *llm* is given)
    only when plan.json is absent.
    """
    units = plan_pipeline.load_plan_units(plan_dir)
    if units is not None:
        return units
    return plan_units(segments, config, plan_dir, llm=llm)[0]
