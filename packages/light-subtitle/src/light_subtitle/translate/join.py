...
def join_cues(
    cues: list[SubtitleCue], plan_units: list[Segment], config: TranslateConfig, *, llm: OpenAIClient | None = None
) -> JoinResult:
    """Repair dangling/flash cues; returns adjusted cues and the (possibly
    split) unit graph.  No-op when there is no LLM client or too few cues."""
    result = JoinResult(cues=cues, units=plan_units)
    if llm is None or len(cues) < 2:
        return result
...
def _max_chars(config: TranslateConfig) -> int:
    ...
    # than the PER-LINE display cap pace enforces (40) — a joined cue may use
    # both lines up to this total.
    return 48
...
def _caps_problems(zh_text: str, start: float, end: float, config: TranslateConfig) -> list[str]:
...
def _validate_ops(
    ops: list[dict], cues: list[SubtitleCue], config: TranslateConfig, core_ids: set[int] | None = None
...
def _validate_merge(op: dict, cues: list[SubtitleCue], config: TranslateConfig) -> str | None:
...
def _validate_shift(op: dict, cues: list[SubtitleCue], config: TranslateConfig) -> str | None:
    i = op["boundary"]
    k = op["en_words"]
...
def _apply_shift(
    ...
    *,
    ...
    texts = _retranslate_pair(prev, nxt, units, config, llm=llm)
    problems = []
    if texts is None:
...
def _retranslate_pair(
    ...
    *,
    ...
    retried, _usage = translate_missing(ordered, {prev.unit_id, nxt.unit_id}, config, llm=llm)
