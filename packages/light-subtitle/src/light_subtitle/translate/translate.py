...
from light_llm.usage.tracker import merge_token_usage, pick_usage_fields
from light_models import Segment, SubtitleCue
from light_text.punctuation import CJK_CLAUSE_PUNCT, SENTENCE_ENDS
...
def _translation_context_fields(config: TranslateConfig) -> dict:
...
def run(
    ...
    *progress*, when given, receives ``(fraction, message)`` after each batch.
    ``llm=None`` means translation is unavailable — returns ``([], None)``.
    """
    if llm is None:
        return [], None
...
def _finalize_translated_cues(cues_1_1: list[SubtitleCue], config: TranslateConfig) -> list[SubtitleCue]:
...
def _build_payload(
    segments: list[Segment], all_segments: list[Segment], batch_idx: int, config: TranslateConfig
) -> dict:
...
def translate_missing(
    ...
    *,
    ...
    ``llm=None`` means translation is unavailable — returns ``([], {})``.
    """
    if llm is None or not missing_ids:
        return [], {}
