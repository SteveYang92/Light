...
def _write_source_exports(orch: Orchestrator, out: Path, formatted: list[SubtitleCue]) -> None:
    ...
    export_module.export_srt(formatted, str(artifacts.sidecar_path(out, f"{ext}.srt")))
    export_module.export_vtt(formatted, str(artifacts.sidecar_path(out, f"{ext}.vtt")))
...
def _write_translated_exports(orch: Orchestrator, out: Path, formatted: list[SubtitleCue]) -> None:
    ...
    if orch.config.annotate:
        export_module.export_annotation_ass(
            ...
            str(artifacts.sidecar_path(out, "annotations.ass")),
        ...
        export_module.export_annotation_vtt(
            formatted,
            orch.state.annotations,
            str(artifacts.sidecar_path(out, "annotations.vtt")),
        )
...
def _write_bilingual_exports(
    ...
    export_module.export_srt(source_fmt, str(artifacts.sidecar_path(out, f"{src_ext}.srt")))
    export_module.export_vtt(source_fmt, str(artifacts.sidecar_path(out, f"{src_ext}.vtt")))

    if target_fmt:
        export_module.export_srt(target_fmt, str(artifacts.sidecar_path(out, f"{tgt_ext}.srt")))
        export_module.export_vtt(target_fmt, str(artifacts.sidecar_path(out, f"{tgt_ext}.vtt")))
        ...
        export_module.export_bilingual_ass(
            source_fmt,
            target_fmt,
            str(artifacts.sidecar_path(out, "bilingual.ass")),
        ...
        export_module.export_bilingual_vtt(
            source_fmt,
            target_fmt,
            str(artifacts.sidecar_path(out, "bilingual.vtt")),
            source_segments=orch.state.composed_segments,
        )
...
def _run_export(orch: Orchestrator) -> None:
    ...
    artifacts.migrate_legacy_sidecars(out, orch.config.slug or None)
