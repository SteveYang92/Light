...
def plan_groups(segments: list[Segment], config: SubtitleConfig) -> tuple[list[list[int]] | None, dict | None]:
    ...
    configured or the LLM output failed validation twice.
    """
    ...
    system = render_prompt("plan_system.j2", max_duration=config.max_duration)
    payload = {
        "segments": [
            {
                "id": i,
    ...
    for attempt in range(_MAX_ATTEMPTS):
        ...
        if groups is None:
            ...
            continue
        problems = _group_problems(groups, segments)
        if not problems:
...
def _group_problems(groups: list[list[int]], segments: list[Segment]) -> list[str]:
    """Hard checks: ordered full coverage, no speaker mixing."""
...
def split_span(words: list[Word], config: SubtitleConfig) -> tuple[list[tuple[int, int]] | None, dict | None]:
    """Split one overlong cue's word span into [start, end) word ranges.

    Returns ``(ranges, usage)``; ``ranges`` is None on failure (caller
    falls back to silence-gap splitting).
    """
    if not config.llm_api_key or len(words) < 2:
        return None, None
    client = OpenAIClient(base_url=config.llm_base_url, api_key=config.llm_api_key, model=config.llm_model)
    system = render_prompt("plan_split_system.j2", max_duration=config.max_duration)
    payload = {
