...
def test_find_video_single_candidate_selected(tmp_path: Path) -> None:
...
def test_find_video_rejects_multiple_videos(tmp_path: Path) -> None:
    # find_video intentionally refuses to guess when several videos are present
    # (mix.py raises RuntimeError and asks for --video); the previous assertion
    # that video.webm is silently preferred never matched this behavior.
    (tmp_path / "other.mp4").write_bytes(b"x")
    (tmp_path / "video.webm").write_bytes(b"x")
    with pytest.raises(RuntimeError, match="Multiple videos found"):
        find_video(tmp_path, None)
...
def test_merge_dub_timeline_trims_overlap(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    sr = 22050
    # write_wav persists 16-bit PCM: samples are clipped to [-1, 1] and quantized,
    # so the two segments use +/-0.5 (the old 1.0/2.0 both clipped to ~1.0 and
    # exceeded allclose's default tolerance). Distinct signs keep the overlap-trim
    # placement check meaningful.
    write_wav(seg1 / "tts" / "dub.wav", np.full(sr * 30, 0.5, dtype=np.float32), sr)
    write_wav(seg2 / "tts" / "dub.wav", np.full(sr * 30, -0.5, dtype=np.float32), sr)
    ...
    assert np.allclose(merged[: sr * 20], 0.5, atol=1e-3)
    assert np.allclose(merged[sr * 20 : sr * 40], -0.5, atol=1e-3)
