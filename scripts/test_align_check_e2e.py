...
Usage::
    ...
    uv run python scripts/test_align_check_e2e.py
    uv run python scripts/test_align_check_e2e.py --case misaligned
    uv run python scripts/test_align_check_e2e.py --dry-run
"""
...
from light_subtitle.llm.client import OpenAIClient, format_token_usage
...
def _run_scenario(scenario: Scenario, config: SubtitleConfig, *, dry_run: bool) -> bool:
    ...
    system_prompt = align_check._render_align_check_prompt(config)
